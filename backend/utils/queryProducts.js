class queryProducts {
    ...
    ratingQuery = () => {
        this.products = this.query.rating ? this.products.filter(c => parseInt(this.query.rating) <= c.rating && c.rating < parseInt(this.query.rating)) : this.products;
    ...
    limit = () => {
        ...
        if ( this.products.length > this.query.perPage) {
            for (i = 0; i < this.query.perPage; i++) {
