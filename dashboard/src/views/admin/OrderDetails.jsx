...
const OrderDetails = () => {
    ...
    const { successMessage, loader, errorMessage, order } = useSelector(state => state.order)
        ...
        <div className='px-2 lg:px-7 pt-5'>
            <div className='w-full p-4 bg-[#6a5fdf] rounded-md'>
                <div className='flex justify-between items-center p-4'>
                    ...
                    <select onChange={(e) => status_update(e)}  value={status} name="" id="" className='px-4 py-2 focus:border-indigo-500 outline-none bg-[#475569] border border-slate-700 rounded-md text-[#d0d2d6]'>
                        ...
                        <div className='w-[30%]'>
                            <div className='pr-3 text-[#d0d2d6] text-lg'>
                                <div className='flex flex-col gap-1'>
                                    ...
                                    <p><span className='text-sm'>{order?.shippingInfo?.address}
                                        {order?.shippingInfo?.area} {order?.shippingInfo?.province} {order?.shippingInfo?.city}
                                    </span></p>
                                </div>
                        ...
                        <div className='w-[70%]'>
                            <div className='pl-3'>
                                <div className='mt-4 flex flex-col bg-[#8288ed] rounded-md p-4'>
                                    {
                                        order.suborders && order.suborders.map((s, i) =>
                                            <div className='text-[#d0d2d6] mt-2'>
                                                <div className='flex justify-start items-center gap-3'>
                                                    <h2>{s.sellerId} Order : </h2>
                                                    <span>{s.delivery_status}</span>
                                                </div>
                                                {
                                                    s.products && s.products.map((product, i) =>
                                                        <div className='flex gap-3 text-md mt-2'>
                                                            <img className='w-[50px] h-[50px]' src={product?.images[0]} alt="" />
