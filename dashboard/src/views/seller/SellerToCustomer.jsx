...
const SellerToCustomer = () => {
    ...
    const { customers, messages, currentCustomer,successMessage,errorMessage } = useSelector(state => state.chat);
                ...
                <div className='flex w-full h-full relative'>
                        ...
                        <div className='w-full h-[calc(100vh-177px)] bg-[#9e97e9] md:bg-transparent overflow-y-auto'>
                            ...
                            </div>
                            {
                                customers?.map((customer, i) =>
                                    ...
                                    <Link to={`/seller/dashboard/chat-customer/${customer?.fdId}`} key={customer?.fdId} className={`h-[60px] flex justify-start gap-2 items-center text-white px-2 py-2 rounded-md cursor-pointer bg-[#8288ed] `}>
                                        <div className='relative'>
                                            <img className='w-[38px] h-[38px] border-white border-2 max-w-[38px] p-[2px] rounded-full' src={customer?.image} alt="" />
                                           
                                           {
                                            activeCustomers.some(c=>c.customerId===customer.fdId) &&  
                    ...
                    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
                        <div className='flex justify-between items-center'>
                            {
                                currentCustomer && <div className='flex justify-start items-center gap-3'>
                                    <div className='relative'>
                                        ...
                                        {
                                            activeCustomers.some(c=>c.customerId===currentCustomer.fdId) &&  
                                            <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
...

                                {
                                    customerId ? messages?.map((m, i) => {
                                        if (m.senderId === customerId) {
                                            return (
                                                <div className='w-full flex justify-start items-center' key={i} ref={scrollToRef}> 
                                            ...
                                            return (
                                                <div className='w-full flex justify-end items-center' key={i} ref={scrollToRef}>
                                                    <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                        ...
                        <form className='flex gap-3'>
                            ...
                            <button  onClick={(e)=> send(e)}   className='shadow-lg bg-[#06b6d4] hover:shadow-cyan-500/50 text-semibold w-[75px] h-[35px] rounded-md text-white flex justify-center items-center'>Send</button>
