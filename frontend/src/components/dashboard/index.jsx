...
import { get_customer_dashboard_data } from '../../store/Reducers/dashboardReducers';

const Index = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const {totalOrders,pendingOrders,cancelledOrders,recentOrders} = useSelector(state => state.dashboard)
                                    ...
                                    recentOrders.map((o, i) => <tr className='bg-white border-b'>
                                        ...
                                        <td scope='row' className='px-6 py-4 font-medium whitespace-nowrap'>
                                            <Link to={`/dashboard/order/details/${o?._id}`}><span className='bg-green-200 text-green-800 text-md font-semibold mr-2 px-3 py-[2px] rounded'>View</span></Link>
                                            {
                                                     ...
                                                     o?.payment_status!=='paid'  &&    <span onClick={()=> redirect(o)} className='bg-green-200 text-green-800 text-md font-semibold mr-2 px-3 py-[2px] rounded'>Pay Now</span>                               
